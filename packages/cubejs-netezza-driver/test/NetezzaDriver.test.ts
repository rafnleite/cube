...
import {
  ...
  interpolateNetezzaParameters,
  NetezzaDriver,
  normalizeNetezzaResultRow,
} from '../src/NetezzaDriver';
import { odbcDiagnostics } from '../src/errors';
...
describe('NetezzaDriver', () => {
  ...
  it('inlines escaped parameters for Netezza ODBC execution', () => {
    expect(interpolateNetezzaParameters(
      "SELECT ? AS number, ? AS text, ? AS timestamp, '?' AS literal, \"?\" AS identifier -- ?\n/* ? */",
      [42, "O'Hara", '2026-08-20T23:59:59.999Z'],
    )).toBe(
      "SELECT 42 AS number, 'O''Hara' AS text, '2026-08-20 23:59:59.999' AS timestamp, '?' AS literal, \"?\" AS identifier -- ?\n/* ? */",
    );
  });

  it('rejects a mismatch between Netezza markers and supplied parameters', () => {
    expect(() => interpolateNetezzaParameters('SELECT ?', [])).toThrow('more parameter markers');
    expect(() => interpolateNetezzaParameters('SELECT 1', ['unused'])).toThrow('more supplied values');
  });

  it('includes Netezza ODBC diagnostics in execution errors', () => {
    expect(odbcDiagnostics(Object.assign(new Error('failed'), {
