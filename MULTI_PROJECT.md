# Multi-project Cube development

This fork keeps semantic models physically and logically isolated:

```
projects/<project-id>/
  project.json       # name and connection preset id; never credentials
  model/
    cubes/*.yml
    views/*.yml
```
...
definitions. Credentials are supplied on each project-open operation and are
held only as AES-256-GCM encrypted values in the backend session store. The
encryption key comes from `CUBEJS_PROJECT_SESSION_SECRET`.
...
The intended API namespace is `/cubejs-api/projects/:projectId/v1`. The project
...
1. Copy `.env.multi-project.example` to `.env.multi-project` and replace the key.
